...
class DecodeConnectorWorker:
    ...
    def _process_zmq(self):
        ...
        async def receiver():
            while not self._resp_stop.is_set():
                ...
                # parse layer tag if provided in request id
                layer_id = None
                if "#L" in req_id:
                ...
                try:
                    if layer_id is not None:
                        logger.warning(
                            "======= successfully get layer id %s for req %s =======",
                            layer_id, req_id
                        )
                        await asyncio.to_thread(self._recv_q.put, (req_id, layer_id))
                    else:
                        # ignore non-layer responses
                        logger.debug("Ignore non-layer response for req %s: %s", req_id, resp)
                except asyncio.CancelledError:
                    break
            ...
            while True:
                try:
                    ...
                    if isinstance(items, tuple):
                        ...
                        logger.warning(
                            "======= in _process_h2d_address: unpack layer id %s for req %s =======",
                            layer_id, req_id
                        )
                    else:
                        req_id = items
                ...
                if layer_id is not None:
                    mb_dev, mb_max, mb_host, mb_sizes = self.omni_cache.build_h2d_ops_layerwise(
                        ctx.local_block_ids,
                        layer_id
                    )
                else:
                    mb_dev, mb_max, mb_host, mb_sizes = self.omni_cache.build_h2d_ops(
                        ctx.local_block_ids,
                        CLUSTER_SIZE
                    )

                batch_device_mem.extend(mb_dev)
                batch_device_max.extend(mb_max)
                batch_host_mem.extend(mb_host)
                batch_host_sizes.extend(mb_sizes)

                ids.append((req_id, layer_id))
    ...
    def _h2d_worker(self):
        ...
        while not self._h2d_stop.is_set():
            batch_device_mem, batch_device_max, batch_host_mem, batch_host_sizes, ids = self._h2d_q.get()
            layer_ids = [str(layer_idx) if layer_idx is not None else "None" for (_, layer_idx) in ids]
            ...
            try:
                ...
                # 2. 累积 per-layer 状态并尝试 finalize
                for req_id, layer_idx in ids:
                    # ctx 用于 log / 结束后清理 pending
                    ctx = self._pending.get(req_id)
                    if not ctx:
                        # 可能已经被 finalize 清掉
                        continue
                    self._record_layer_done_and_maybe_finalize(req_id, ctx, layer_idx)

                logger.warning("======= in _h2d_worker: successfully post_process layers %s =======", ",".join(layer_ids))
            except Exception as e:
                logger.exception(
                    "H2D worker error on req_ids=%s: %s",
                    ",".join([rid for (rid, _) in ids]), e
                )
            finally:
                t_h2d_end = time.time()
                logger.debug(
    ...
    def _record_layer_done_and_maybe_finalize(self, req_id: str, ctx: 'PendingReq', layer_idx: Optional[int]):
        """
        在主进程的 _layer_state[req_id] 中累积 per-layer 状态，然后基于“所有 num_layers 层必须完成”的规则尝试 finalize。
        """
        num_layers = self.omni_cache.num_layers

        # 初始化该 req_id 的状态集合
        s = self._layer_state.get(req_id)
        if s is None:
            s = set()
            self._layer_state[req_id] = s

        if layer_idx is None:
            # full copy: 直接认为所有层 done
            s.clear()
            s.update(range(num_layers))
            logger.warning("======= done h2d full-copy for req %s ======", req_id)
        else:
            s.add(layer_idx)
            logger.warning("======= done h2d copy for layer %s req %s, layers_done_now=%s ======",
                           layer_idx, req_id, sorted(list(s)))

        # 严格语义：所有 0..(num_layers-1) 层都必须出现在 s 里
        self._maybe_finalize(req_id, ctx, s, num_layers)

    def _maybe_finalize(self, req_id: str, ctx: 'PendingReq', layers_done: set[int], num_layers: int):
        """
        只有当 0..(num_layers-1) 所有层都在 layers_done 中时，
        才触发：
        ...
        # 防止重复 finalize，同一个 req_id 只执行一次
        ...
        try:
            ...
            # 完成后清理 pending 和 layer_state
            try:
                self._pending.pop(req_id, None)
            except Exception:
                pass
            self._layer_state.pop(req_id, None)
